...
namespace Sunflowerology.Common.GlobalProjectiles
{
    internal class SnowflowerProjBonus : GlobalProjectile
    {
        ...
        private const int SnowflakeMinDistance = 16*20;
        ...
        public override void PostAI(Projectile projectile)
        {
            ...
            if (posLastSnowflake.HasValue &&
                (posLastSnowflake.Value - projectile.Center).Length() > SnowflakeMinDistance)
            {
        ...
        private void TrySpawnTrail(Projectile projectile)
        {
            ...
            int count = Math.Min(10, (int)(distance / SnowflakeMinDistance));
                    ...
                    !player.ghost)
                {
                    for (int j = 0; j <= 5; j++)
                    {
                        var p = Projectile.NewProjectile(projectile.GetSource_FromThis(), newPos,
                                ...
                                (int)Math.Ceiling(projectile.damage / 30f), 0, projectile.owner, Main.rand.Next(3));
                    }
                }
